...
void init_1() {
  name_.push_back("Young");
  name_.push_back("Pre-presbyopic");
  name_.push_back("Presbyopic");
  name_.push_back("Hypermetrope");
  name_.push_back("Astigmatism");
  name_.push_back("Normal Tear Rate");
  name_.push_back("Contact Lenses");
  for (int i = 0; i < name_.size(); i++)
    for (int j = 0; j < name_[i].size(); j++) {
      name[i][j] = name_[i][j];
    }
}
...
void init_2() {
  ...
  name_.push_back("Lmost ");
  name_.push_back("Lmost1 ");
  name_.push_back("Lmost2 ");
  name_.push_back("Lmost3");
  name_.push_back("Lmost4");
  name_.push_back("Lmost5");
  name_.push_back("Lmost6");
  name_.push_back("Lmost7");
  name_.push_back("Lmost8");
  name_.push_back("Lmost9");
  name_.push_back("Lmost10");
  name_.push_back("Lmost11");
  name_.push_back("Lmost12");
  name_.push_back("Lmost13");
  name_.push_back("Lmost14 ");
  name_.push_back("Rmost14");
  name_.push_back("Rmost13");
  name_.push_back("Rmost12");
  name_.push_back("Rmost11");
  name_.push_back("Rmost10");
  name_.push_back("Rmost9");
  name_.push_back("Rmost8");
  name_.push_back("Rmost7");
  name_.push_back("Rmost6");
  name_.push_back("Rmost5");
  name_.push_back("Rmost4 ");
  name_.push_back("Rmost3");
  name_.push_back("Rmost2");
  name_.push_back("Rmost1");
  name_.push_back("Rmost ");
  name_.push_back("Symm");
  for (int i = 0; i < name_.size(); i++)
    for (int j = 0; j < name_[i].size(); j++) {
      name[i][j] = name_[i][j];
    }
  ...
  for (int i = 0; i < frequents -> size(); i++) {
    ...
    for (int j = 0; j < ( * frequents)[i].size(); j++) {
      ...
      for (k = 0; k < ( * frequents)[i][j] -> items.size() - 1; k++) {
        printf("  %s  ,", name[( * frequents)[i][j] -> items[k]]);
        ...
        if (cnt == set_1.size() && fre[i] == fre[j]) {
          ...
          if (TRY == 0) {
            ...
            for (int idx = 0; idx < set_1.size(); idx++)
              printf("  %s  ", name[set_1[idx]]);
            printf(" } ");
            printf("->");
            printf(" { ");
            for (int idx = 0; idx < set_2.size(); idx++)
              printf("  %s  ", name[set_2[idx]]);
            printf(" }  ");
            printf("\n");
